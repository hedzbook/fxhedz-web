...
<div className="flex flex-col flex-1 min-h-0 bg-neutral-900 border border-neutral-800 p-[clamp(8px,1vw,14px)] text-[clamp(9px,5.5px+1.0937vw,19.5px)]">
  ...
  {/* HEADER */}
  <div className="grid grid-cols-[90px_120px_60px_80px_60px_100px] text-neutral-400 mb-2 font-mono tabular-nums">
    ...
    <div className="px-2 text-right tracking-tight">
      {data?.orders?.reduce((s:any,o:any)=>s+Number(o.lots||0),0).toFixed(2)}
    </div>

    <div className="px-2 text-right">~PnL</div>
    <div className="px-2 text-right tracking-tight">
      {data?.orders?.reduce((s:any,o:any)=>s+Number(o.profit||0),0).toFixed(2)}
    </div>
  ...
  <div className="flex-1 overflow-y-auto space-y-1">
    ...
    {data?.orders?.length ? data.orders.map((o:any,i:number)=>(
      ...
      <div
        key={i}
        className="grid grid-cols-[90px_120px_60px_80px_60px_100px] bg-neutral-800 py-1 font-mono tabular-nums"
      >
